...
/*
  document.addEventListener("DOMContentLoaded", function() {
    const svg = document.getElementById("mapa-svg");
    const botonesDepartamento = svg.querySelectorAll(".department");
    const botonMostrarMapaCompleto = document.getElementById("mostrar-mapa-completo");
    ...
    // Mostrar departamento y ajustar zoom
    function mostrarDepartamento(departamentoId) {
      // Obtener el departamento seleccionado
      const departamento = svg.querySelector(`[data-valor="${departamentoId}"]`);
  
      if (departamento) {
        // Ocultar botones de departamento
        botonesDepartamento.forEach((boton) => {
          boton.style.display = "none";
        });
  
        // Ajustar viewBox para enfocarse en el departamento
        const bbox = departamento.getBBox();
        svg.setAttribute("viewbox", `${bbox.x} ${bbox.y} ${bbox.width} ${bbox.height}`);
  
        // Mostrar botón para volver al mapa completo
        botonMostrarMapaCompleto.style.display = "block";
      }
    }
    ...
    // Manejador de clic para el botón "Mostrar Mapa Completo"
    botonMostrarMapaCompleto.addEventListener("click", () => {
      // Mostrar todos los botones de departamento
      botonesDepartamento.forEach((boton) => {
        boton.style.display = "block";
      });
  
      // Restaurar el tamaño y vista original del SVG
      svg.setAttribute("viewbox", "0 0 1000 1056");
